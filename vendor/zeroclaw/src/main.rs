...
Examples:
  ...
  zeroclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
        ...
        /// Restrict the agent to the specified tool names (repeatable)
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "agent-runtime")]
    fn agent_command_parses_allowed_tools() {
        let cli = Cli::try_parse_from([
            "zeroclaw",
            "agent",
            "--allowed-tool",
        ...
        .expect("agent command with allowed tools should parse");

        match cli.command {
            Commands::Agent { allowed_tools, .. } => {
                assert_eq!(allowed_tools, vec!["file_read", "content_search"]);
            }
            other => panic!("expected agent command, got {other:?}"),
        }
    }
